...
import { sanitizeText, sanitizeEmail } from '../../shared/utils/sanitize.js';
...
function DemoForm({ children, onSubmitLabel = 'Sent — thank you!', className, style }) {
  ...
  const submit = (e) => {
    e.preventDefault();
    const form = e.target;
    form.querySelectorAll('input[type="text"], textarea').forEach((el) => { el.value = sanitizeText(el.value); });
    form.querySelectorAll('input[type="email"]').forEach((el) => { el.value = sanitizeEmail(el.value); });
    setSent(true);
    form.reset();
  ...
  return (
    <form className={className} style={style} onSubmit={submit}>
      {children}
      <button type="submit" className="btn btn-primary btn-block" style={{ marginTop: '24px' }} disabled={sent}>{sent ? onSubmitLabel : 'Submit'}</button>
    </form>
  );
}
...
export default function Admissions() {
  const faqItems = [
    { q: 'Is there a waiting list?', a: 'Popular entry grades run waiting lists most years — we recommend applying at least one term ahead.' },
    { q: 'Do you offer bursaries or sibling discounts?', a: 'Yes — a 10% sibling discount applies from the second child enrolled, and merit bursaries are available.' },
    { q: 'Can my child join mid-year?', a: 'Mid-year transfers are welcome, subject to space and a short placement assessment.' },
    { q: 'Is transport available from my area?', a: 'Our school buses cover most routes within Kapsabet town and surrounding trading centres.' },
  ];

  const admissionsSchema = { "@context": "https://schema.org", "@type": "EducationalOrganization", "name": "Baraka School Kapsabet Admissions", "description": "Admissions process, requirements, and fee structure for Playgroup through Grade 9." };

  return (
    <>
      <SEO title="Admissions, Fees & Requirements" description="Learn how to apply to Baraka School Kapsabet. View our termly fee structure, admission requirements, and start your online application." schema={admissionsSchema} />
      <PageHero crumb="Admissions" title="Begin your child's Baraka journey">Places for Playgroup through Grade 9 are open for the 2027 intake.</PageHero>

      <section className="bg-flush section-tight">
        <div className="container hero-cta" style={{ justifyContent: 'center' }}><a href="#apply" className="btn btn-primary">Apply Online</a></div>
      </section>
      ...
      <section id="process" className="bg-flush">
        <div className="container">
          ...
          <div className="steps stagger">
            {[['01', 'Submit Enquiry', 'Fill the online form or visit our admissions office.'], ['02', 'Assessment & Interview', 'A short, friendly placement assessment.'], ['03', 'Offer & Enrolment', 'Receive your offer letter, pay the deposit, confirm your place.']].map(([num, title, body], i) => (<Reveal key={num} className="step" delay={i * 80}><span className="num">{num}</span><h3>{title}</h3><p>{body}</p></Reveal>))}
          </div>
        </div>
      </section>
      ...
      <section className="bg-tint">
        <div className="container split">
          ...
          <Reveal dir="right" className="split-media"><Tile swatch="p2" bgImage="https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?auto=format&fit=crop&w=800&q=80" alt="Students writing" className="frame" loading="lazy" /></Reveal>
        </div>
      </section>

      <section id="fees" className="bg-flush">
        <div className="container">
          <Reveal className="section-head"><p className="eyebrow">Investment</p><h2>Fees overview</h2></Reveal>
        ...
        <div className="container">
          ...
          <Reveal>
            <DemoForm className="card-form" style={{ maxWidth: '820px', marginInline: 'auto' }}>
              ...
              <div className="field"><label htmlFor="pgrade">Grade Applying For <span className="req">*</span></label><select id="pgrade" required defaultValue=""><option value="" disabled>Select grade</option><option>Playgroup</option><option>PP1 / PP2</option><option>Grade 1 – 3</option><option>Grade 4 – 6</option><option>Grade 7 – 9</option></select></div>
